...
/// Surfaces every [Level#SEVERE] log record from MinDis's own code as an
/// error dialog, in addition to whatever [org.mindis.core.logging.LoggingBootstrap]
/// already writes to console/file (every record, not just SEVERE, and not
/// just MinDis's own, still reaches [LogConsoleHandler]'s in-app
/// history). Restricted to `org.mindis` logger names - third-party code
...
///
/// <p>Content is a plain, non-editable [TextArea] rather than
/// [Alert#setContentText], whose text is a `Label` - not
