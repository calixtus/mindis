...
/// document: its own identity ([CollectionMeta] - name and logo), the
...
///
/// <p>Null-tolerant like the model records ([Server], [org.mindis.core.model.Slot]): a list
/// absent from an older or hand-edited
/// file reads as empty, and a missing `meta` block reads as [CollectionMeta#empty()], rather
...
public record MinDisDocument(
    ...
    /// Bumped to 2 when [CollectionMeta] was added; a v1 document simply
    /// lacks the field, which the compact constructor fills with [CollectionMeta#empty()] - no
    ...
    /// A document with no data at all - the starting point of [AppDatabase#newDocument()],
    /// which seeds the default roles into it.
