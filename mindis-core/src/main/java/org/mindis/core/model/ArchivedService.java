...
/// A frozen, self-contained snapshot of a [LiturgicalService] and its
...
/// [ArchivedSlot]). Deleting or renaming a [Server] or [Role]
...
public record ArchivedService(
    ...
    /// as the display names captured at archive time. `serverName` is
    /// `null` for a slot that was left open. `serverId` is kept
    /// only so cross-boundary spacing continuity can still match a server by
    /// identity ([org.mindis.core.planning.PriorAssignment]); it is never
