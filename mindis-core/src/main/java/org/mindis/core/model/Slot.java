...
/// One physical role slot of a [LiturgicalService] - unlike
/// [RoleSlot] (a role/count requirement on a [ServiceTemplate]),
/// each `Slot` is its own individually-identified instance, one per
...
///
/// <p>The assignment lives directly on the slot: `serverId` is the
/// [Server#id()] filling it (`null` = still open), `pinned`
...
/// its own assignments, so saving a service saves its plan. `serverId`/
...
public record Slot(String id, String role, @Nullable String serverId, boolean pinned) {
    ...
    /// [RoleSlot]. Used when a template's slots are copied onto a new
    /// [LiturgicalService] (see `ServiceGenerator`); a template's
    ...
    /// The inverse of [#expand] - collapses individual instances back
    ...
    /// True if `a` and `b` carry the same assignments, ignoring
