...
/// The date pattern of a [ServiceTemplate]: a pure predicate over
...
///
/// <p>The vocabulary is deliberately small and combined with [AllOf],
/// [AnyOf] and [Not] rather than grown by special cases:
/// <ul>
///   <li>"every third Sunday" = `allOf(nthWeekdayOfMonth(3, SUNDAY))` -
///       the ordinal rule already carries the weekday</li>
///   <li>"every other Sunday" = `allOf(weekly(SUNDAY), everyNWeeks(2, anchor))`</li>
///   <li>"the 13th, and every Sunday" = `anyOf(dayOfMonth(13), weekly(SUNDAY))`</li>
///   <li>"every Sunday except Christmas" =
///       `allOf(weekly(SUNDAY), not(fixedMonthDay(DECEMBER, 25)))`</li>
...
/// date is a pure function of the year (see [LiturgicalCalendar]), so
/// [FeastRelative] stays a date predicate like every other rule.
///
...
/// `RecurrenceRuleMixin` for the JSON document, `RecurrenceCodec`
...
public sealed interface RecurrenceRule {
    ...
    /// How far [#nextOccurrences] looks ahead before giving up. A rule
    /// may match nothing at all ([Never], or a combination that cancels
    ...
    /// True if a service occurs on `date`. Must be side-effect free and
    /// depend on nothing but the date itself.
    boolean matches(LocalDate date);

    /// The first `limit` dates this rule matches on or after
    /// `from`, for a preview of what a template will generate. Fewer
    /// than `limit` dates - possibly none - if the rule runs out within
    ...
    /// "The n-th `day` of the month", n counted from the month's end
    /// when negative (`-1` = last).
    ...
    /// A feast day itself: `feast(EASTER, 0)`, or shifted by whole days:
    /// `feast(EASTER, -2)` is Good Friday.
    ...
    /// the month's end, `-1` being its last day. A number that a given
    ...
    /// A month filter, only useful in combination: an [AllOf] of this
    ...
    /// leap years only, for the same reason [DayOfMonth] does not clamp.
    ...
    /// Every `n`-th day counted from `anchor`, in both directions
    ...
    /// Every `n`-th week counted from the anchor's week - "every other
    ...
    /// Every `n`-th month counted from the anchor's month - quarterly,
    /// half-yearly. Combine with [DayOfMonth] or
    /// [NthWeekdayOfMonth] to pick the day within the month.
    ...
    /// Saturday before the first Sunday of Advent (`ADVENT_1 - 1`).
    ///
    ...
    /// Negation, used inside an [AllOf] as an exception: "every Sunday
